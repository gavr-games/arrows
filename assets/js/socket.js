...
if (window.gameId) {
  channel = socket.channel("game:" + window.gameId, {})
  channel.join()
    .receive("ok", resp => { 
  ...
  channel.on("player2_joined", payload => {
    let player2Name = document.getElementById("player-2-name")
    player2Name.innerText = payload.name
  })
  ...
  channel.on("update_arrow", payload => {
    game.updateArrow(payload.arrow)
  })
}
